...
class LexiconEncoder(nn.Module):
    def __init__(self, config, embedding=None, use_char_emb=False, use_char_rnn=True, use_exm=True, use_cove=True, elmo_on = True):
        ...
        # Add the elmo
        self.elmo_size = self.create_elmo(config)
        self.output_size += self.elmo_size * 2
    ...
    def create_elmo(self, config):
        ...
        size = config['elmo_size']
        self.elmo_on = elmo_on
        if elmo_on:
            self.elmo = Elmo(config['elmo_config_path'], config['elmo_weight_path'], num_layer, dropout=config['elmo_dropout'])
        else:
            self.elmo = None
            size = 0
        return size
