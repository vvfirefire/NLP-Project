...
class BatchGen(object):
    def __init__(self, config, raw_data: dict, label, batch_size, is_training=True, augment=False, elmo_on = True):
        self.config = config
    ...
    def __iter__(self):
        ...
        while self.offset < len(self):
            ...
            if self.elmo_on:
                doc_cid = torch.LongTensor(N, ld, ELMoCharacterMapper.max_word_length).fill_(0)
                query_cid = torch.LongTensor(N, lq, ELMoCharacterMapper.max_word_length).fill_(0)
            ...
            batch_dict['doc_mask'] = self.patch(~(torch.eq(doc_tok_tensor, 0)))
            batch_dict['doc_char'] = self.patch(doc_char_tensor)
            batch_dict['doc_char_mask'] = self.patch(~(torch.eq(doc_char_tensor, 0)))
            ...
            # Add Elmo
            if self.elmo_on:
                batch_dict['doc_ctok'] = self.patch(doc_cid)
                batch_dict['query_ctok'] = self.patch(query_cid)

            batch_dict['query_tok'] = self.patch(query_tok_tensor)
            batch_dict['query_mask'] = self.patch(~(torch.eq(query_tok_tensor, 0)))
            batch_dict['query_char'] = self.patch(query_char_tensor)
            batch_dict['query_char_mask'] = self.patch(~(torch.eq(query_char_tensor, 0)))
